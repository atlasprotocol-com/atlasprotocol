...
impl Atlas {
    ...
    pub fn new(
        ...
        assert!(
            !global_params_owner_id.to_string().is_empty(),
    ...
    pub fn change_atlas_admin_id(&mut self, new_admin_id: AccountId) {
        ...
        assert!(
            !new_admin_id.to_string().is_empty(),
            "New admin ID cannot be blank"
        );
        assert_ne!(
            new_admin_id, self.admin_id,
            "New admin ID must be different from the current admin ID"
        );
