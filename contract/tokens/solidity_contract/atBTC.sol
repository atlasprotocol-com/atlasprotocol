...
contract atBTC is ERC20, ERC20Burnable, ERC20Pausable, Ownable {
    ...
    // Function to pause the contract (external visibility for gas optimization)
    function pause() external onlyOwner {
        _pause();
    }

    // Function to unpause the contract (external visibility for gas optimization)
    ...
    // Mint function for deposit bitcoin, only accessible to the owner when the contract is not paused (external visibility for gas optimization)
    function mintDeposit(address to, uint256 amount, string memory btcTxnHash) external onlyOwner whenNotPaused {
        _mint(to, amount); // Internal mint function
        emit MintDeposit(to, btcTxnHash, amount);
    }

    // Mint function for bridging atlas bitcoin, only accessible to the owner when the contract is not paused (external visibility for gas optimization)
    function mintBridge(address to, uint256 amount, string memory originChainId, string memory originChainAddress, string memory originTxnHash) external onlyOwner whenNotPaused {
        _mint(to, amount); // Internal mint function
        emit MintBridge(to, originChainId, originChainAddress, amount, originTxnHash);
    }

    // Custom burn function that logs the btcAddress for the redemption process (external visibility for gas optimization)
    function burnRedeem(uint256 amount, string memory btcAddress) external whenNotPaused {
        _burn(msg.sender, amount);
        emit BurnRedeem(msg.sender, btcAddress, amount);
    }

    // Custom burn function that logs the destination chainId and address for the bridging process (external visibility for gas optimization)
