...
async fn test_deposit_verification() {
    ...
    assert!(signet_chain_config.is_some(), "SIGNET chain configuration should be present");
    ...
    assert_eq!(deposit.status, DEP_BTC_DEPOSITED_INTO_ATLAS, "Expected status to be DEP_BTC_DEPOSITED_INTO_ATLAS");
}
...
async fn test_deposit_verification_with_invalid_data() {
    ...
    assert!(!verified, "Expected deposit verification to fail with invalid data");
}
...
async fn test_deposit_verification_with_multiple_validators() {
    ...
    assert!(verified_again, "Expected deposit verification to succeed with second validator");
...
async fn test_get_chain_ids_by_validator_and_network_type() {
    ...
    let chain_ids = atlas.get_chain_ids_by_validator_and_network_type(accounts(4), "SIGNET".to_string());
...
async fn test_get_all_validators() {
    ...
    assert!(all_validators.iter().any(|(account, chains)| 
        *account == accounts(4) && chains.contains(&"SIGNET".to_string()) && chains.contains(&"TESTNET".to_string())
    ));
    assert!(all_validators.iter().any(|(account, chains)| 
        *account == accounts(5) && chains == &vec!["SIGNET".to_string()]
    ));
}
