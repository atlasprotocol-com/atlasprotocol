...
async function MintBridgeABtcToDestChain(allBridgings, near) {
  if (flagsBatch.BridgeaBtcToDestChainRunning) {
    ...
    try {
      ...
      const allBridgingsToBridge =
        await getBridgingRecordsToBridge(allBridgings);
      console.log(
        `${batchName} Found ${allBridgingsToBridge.length} bridging to bridge.`,
      );
      ...
      const transactionsByChain = allBridgingsToBridge.reduce(
        (acc, bridging) => {
      ...
      for (const [chainId, chainTransactions] of Object.entries(
        ...
        if (chainConfig.networkType === NETWORK_TYPE.EVM) {
          ...
          for (const bridging of chainTransactions) {
            ...
            console.log(
              `[mintBridgeABtcToDestChain] Processing record ${currentIndex} of ${totalRecords} for chain ${chainId}: bridging with txn hash ${txnHash}`,
            );

            try {
              if (bridging.verified_count < chainConfig.validators_threshold) {
                throw new Error(
                  `Verified count is less than validators threshold: ${bridging.verified_count} < ${chainConfig.validators_threshold}`,
                );
              }

              if (
                !address.isValidEthereumAddress(bridging.dest_chain_address)
              ) {
                throw new Error(
              ...
              console.log(
                `${batchName} Bridge record: ${JSON.stringify(bridging)}`,
              );
              const finalAbtcAmount =
              ...
              const { nonce, signed } =
                await ethereum.createMintBridgeABtcSignedTx(
                  near,
                  txnHash,
                  sender,
              ...
              const { txnHash: evmTxnHash, status } =
