...
const getBtcMempoolRecords = async () => {
  try {
    ...
    console.error(
      `[getBtcMempoolRecords] Failed to fetch Btc Mempool records: ${error.message}`,
    );
  }
};
...
app.get("/api/v1/process-new-bridging", async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: `Successfully processed bridging for transaction ${txnHash}`,
    });
  } catch (error) {
    console.error("Error processing new bridging:", error);
    res.status(500).json({
      error: "Failed to process new bridging",
      details: error.message,
    });
  }
});
