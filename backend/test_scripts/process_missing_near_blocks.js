...
const CONFIG = {
  ...
  DEPOSITS_QUEST2: {
    ...
    START_INDEX: 3720,                     // Starting row index (0-based, excluding header)
...
async function checkLPShares(accountId, poolId) {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      const result = await new Promise((resolve, reject) => {
        ...
        exec(command, (error, stdout, stderr) => {
          ...
          if (error || stderr) {
            ...
            if (stderr.includes("update check failed")) {
              console.log(`Attempt ${attempt}/${maxRetries}: Timeout error for ${accountId} pool ${poolId}, retrying...`);
          ...
          try {
            ...
            console.error(`Error parsing output for ${accountId} pool ${poolId}:`, parseError);
            reject(parseError);
          }
        });
      });
      ...
      if (error.message === "TIMEOUT_ERROR" && attempt < maxRetries) {
  ...
  throw new Error(`Failed to check LP shares after ${maxRetries} attempts`);
}
